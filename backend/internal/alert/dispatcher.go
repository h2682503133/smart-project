...
func NewDispatcher(store EventStore, client *http.Client, notifyURL, serviceKey string) (*Dispatcher, error) {
	if store == nil || client == nil || strings.TrimSpace(notifyURL) == "" || strings.TrimSpace(serviceKey) == "" {
		return nil, errors.New("alert dispatcher dependencies are required")
	}
	parsed, err := url.ParseRequestURI(strings.TrimSpace(notifyURL))
	if err != nil || parsed.Scheme != "http" && parsed.Scheme != "https" || parsed.Host == "" {
		return nil, errors.New("alert notify URL must be an absolute HTTP URL")
	}
	return &Dispatcher{
...
func (d *Dispatcher) DispatchOnce(ctx context.Context, batchSize int) (int, error) {
	if batchSize < 1 || batchSize > 500 {
		return 0, errors.New("alert dispatcher batch size must be between 1 and 500")
	}
	now := d.now().UTC()
	events, err := d.store.ClaimAvailableByEventPrefix(ctx, now, "ALERT_", batchSize, 30*time.Second)
	if err != nil {
		return 0, fmt.Errorf("claim alert outbox events: %w", err)
	}
	published := 0
	...
	for index := range events {
		if err := d.deliver(ctx, &events[index]); err != nil {
			...
			availableAt := failureTime.Add(alertRetryDelay(events[index].RetryCount + 1))
			if markErr := d.store.MarkFailed(ctx, events[index].ID, message, availableAt, failureTime); markErr != nil {
				dispatchErrors = append(dispatchErrors, fmt.Errorf("mark alert event %d failed: %w", events[index].ID, markErr))
			}
			dispatchErrors = append(dispatchErrors, fmt.Errorf("deliver alert event %d: %w", events[index].ID, err))
			continue
		}
		if err := d.store.MarkPublished(ctx, events[index].ID, now); err != nil {
			dispatchErrors = append(dispatchErrors, fmt.Errorf("mark alert event %d published: %w", events[index].ID, err))
			continue
		}
		published++
	}
...
func (d *Dispatcher) Run(ctx context.Context, interval time.Duration, batchSize int) {
	...
	for {
		if _, err := d.DispatchOnce(ctx, batchSize); err != nil && !errors.Is(err, context.Canceled) {
			slog.Warn("dispatch alert outbox", "error", err)
		}
		select {
		case <-ctx.Done():
			return
...
func alertRetryDelay(retryCount int) time.Duration {
	if retryCount < 1 {
		retryCount = 1
	}
	if retryCount > 8 {
